...
| 字段 | 类型 | 说明 |
|------|------|------|
| data.embedding | List[float] | 文本对应的向量 |
| data.dimension | int | 向量维度，取决于配置的 embedding 模型 |
...
{
    "items": [
        {"id": "article_101", "text": "gut microbiome"},
        {"id": "article_102", "text": "intestinal flora"}
    ]
}
```
...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| items[].id | string | 是 | 文本唯一标识，用于对应返回结果 |
| items[].text | string | 是 | 待转换的文本 |
...
```json
{
    ...
    "message": "success",
    "data": [
        {"id": "article_101", "embedding": [0.012, -0.034, ...]},
        {"id": "article_102", "embedding": [0.078, -0.012, ...]}
    ],
    "dimension": 1024,
    "count": 2
}
