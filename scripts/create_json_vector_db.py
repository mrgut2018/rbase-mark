...
from deepsearcher.tools.log import error, info, warning
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Load article data from JSON file to vector database"
    )
    parser.add_argument("--file", "-f", required=True, help="Dataset JSON file path to load")
    parser.add_argument("--collection", "-c", required=True, help="Collection name in vector database")
    parser.add_argument(
        "--collection-description", "-d", help="Collection description in vector database"
    )
    ...
    if args.file:
        ...
        json_file_path = os.path.join(
            current_dir, "..", "database", "json", "akk_sample_processed.json"
        )
    ...
    if args.collection_description:
        ...
        collection_description = (
            f"Academic articles dataset imported from {os.path.basename(json_file_path)}"
        )
    ...
    info(
        f"Successfully loaded articles from JSON file into vector database collection '{collection_name}'"
    )
