...
from deepsearcher.api.models import VectorDbOperationStr
from deepsearcher.rbase_db_loading import insert_to_vector_db, save_vector_db_log, log_raw_article_deleted
from deepsearcher.rbase_db_loading import load_markdown_articles, load_non_full_text_articles
from deepsearcher.tools.log import info, error, set_dev_mode, set_level
...
def main(ver: int, env: str, collection_name: str, offset: int = 0, limit: int = 10, **kwargs):
    ...
    is_full_text = kwargs.get("is_full_text", True)
    ...
    if config.rbase_settings.get("env"):
        env = config.rbase_settings.get("env")
    ...
    if is_full_text:
        ...
        articles = load_non_full_text_articles(config.rbase_settings, offset=offset, limit=limit, base_id=base_id, doc_rebuild=doc_rebuild)
    ...
    for article in tqdm(articles, desc="Processing articles"):
        ...
        if insert_result:
            ...
            save_vector_db_log(config.rbase_settings, 
                               ...
                               collection_name, 
                               operation=VectorDbOperationStr.INSERT if is_full_text else VectorDbOperationStr.ABSTRACT,
                               id_from=min_id,
                               id_to=max_id)
...
def parse_args():
    ...
    parser.add_argument('--env', '-e', type=str, default='', help='环境名称，dev或prod，如果config.rbase_settings.env已配置则传参无效')
    ...
    parser.add_argument('--collection_description', '-d', type=str, default='Contains a collection of professional literature in the fields of biomedicine, medical care, health, etc.', 
                        help='集合描述')
    ...
    parser.add_argument('--is_abstract', '-a', action='store_true', help='是否只处理摘要，默认为False')
...
if __name__ == "__main__":
    ...
    main(args.ver, args.env, args.collection_name, args.offset, args.limit, 
        ...
        is_full_text=not args.is_abstract)
