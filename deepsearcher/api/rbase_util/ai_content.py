...
async def save_ai_log(log: AILog) -> int:
    """
    保存AI日志到数据库（同步版本）
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                if log.id == 0:
                    ...
                    sql = """
                    INSERT INTO ai_log (
                        log_type, uuid, intention, resource_log, search_query, query_limit, 
                        intention_duration, search_duration, total_duration
                    ) VALUES (
                        %s, %s, %s, %s, %s, %s, %s, %s, %s
                    )
                    """
        ...
        raise Exception(f"Failed to save AI log to db: {e}")