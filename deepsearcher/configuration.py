...
FeatureType = Literal[
    "llm", "embedding", "file_loader", "web_crawler", "vector_db", "reasoning_llm", "lctx_reasoning_llm", "writing_llm"
]
...
class ModuleFactory:
    ...
    def create_lctx_reasoning_llm(self) -> BaseLLM:
        return self._create_module_instance("lctx_reasoning_llm", "deepsearcher.llm")
...
lctx_reasoning_llm: BaseLLM = None
...
def init_config(config: Configuration) -> bool:
    ...
    log.debug("initializing lctx_reasoning_llm(long context support)")
    if "lctx_reasoning_llm" in config.provide_settings:
        lctx_reasoning_llm = module_factory.create_lctx_reasoning_llm()
    else:
        lctx_reasoning_llm = reasoning_llm # Fallback to the default LLM if not configured
...
def init_rbase_config(config_file: str = None) -> Configuration:
    """
    初始化 rbase 配置的便捷函数

    Args:
        config_file: 配置文件路径，如果为 None 则使用默认的 config.rbase.yaml

    Returns:
        Configuration: 初始化后的配置对象
    """
    global config

    if config_file is None:
        # 默认使用项目根目录下的 config.rbase.yaml
