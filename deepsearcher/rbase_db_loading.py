...
from deepsearcher.api.models import VectorDbOperation, VectorDbOperationStr, vector_db_operation_to_int
...
def insert_to_vector_db(rbase_config: dict, 
    ...
    try:
        with conn.cursor() as cursor:
            ...
            for article in articles:
                txt_file_path = article.txt_file if is_full_text else ""
                
                # Additional check if txt_file_path is empty or not ending with .md
                if is_full_text and (not txt_file_path or not txt_file_path.endswith('.md')):
                ...
                with tempfile.NamedTemporaryFile(suffix='.md', delete=False) as temp_file:
                    ...
                    if is_full_text and (not host or not txt_file_path):
                    ...
                    try:
                        if is_full_text:
                            ...
                            if not content or content == "":
                                warning(f"Skipping empty abstract content of article: {article.article_id}")
                                continue
                        
                        temp_file.write(content.encode('utf-8'))
                        
                        # 在database/markdown/目录下存储备份文件
                        if save_downloaded_file and is_full_text:
                ...
                for doc in docs:
                    ...
                    author_names = [name[:95] if len(name) > 100 else name for name in author_names]
                    ...
                    corresponding_author_names = [name[:95] if len(name) > 100 else name for name in corresponding_author_names]
...
def load_markdown_articles(rbase_config: dict, offset: int = 0, limit: int = 10, **kwargs) -> list[RbaseArticle]:
    """
    Load article data with markdown files from Rbase database 
    ...
    try:
        with conn.cursor() as cursor:
            ...
            if not doc_rebuild:
                rebuild_cond = "ra.id NOT IN (SELECT raw_article_id FROM vector_db_data_log WHERE status=1 AND operation=%d) "
                sql += "\n AND " + rebuild_cond % (VectorDbOperation.INSERT.value)
...
def load_non_full_text_articles(rbase_config: dict, offset: int = 0, limit: int = 10, **kwargs) -> list[RbaseArticle]:
    """
    Load article data (abstract data)with non-full-text files from Rbase database 
    ...
    try:
        with conn.cursor() as cursor:
            ...
            FROM raw_article ra
            LEFT JOIN article a ON ra.id = a.raw_article_id
            WHERE a.status=1 AND (ra.txt_file IS NULL OR ra.txt_file = '' OR ra.txt_file LIKE '%%.txt') """
            if not doc_rebuild:
                rebuild_cond = "ra.id NOT IN (SELECT raw_article_id FROM vector_db_data_log WHERE status=1 AND operation=%d) "
                sql += "\n AND " + rebuild_cond % (VectorDbOperation.INSERT.value)
            if isinstance(base_id, int) and base_id > 0:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                SELECT a.id, a.raw_article_id, a.txt_file, 
                    ...
                    a.summary as abstract, a.csummary as chinese_abstract,
                    a.journal_name, a.summarize as summary, a.type
                FROM article a
...
async def load_article_by_article_id(article_id: int, article_uuid: str = "") -> RbaseArticle:
    """
    Load article data from Rbase database, article_id and article_uuid are mutually exclusive.
    If article_uuid is provided, article_id will be ignored.
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                if article_uuid:
                    sql += " WHERE a.uuid=%s"
                    await cursor.execute(sql, (article_uuid,))
                else:
                    sql += " WHERE a.id=%s"
                    await cursor.execute(sql, (article_id,))
                article_data = await cursor.fetchone()
                return RbaseArticle(article_data)
    except Exception as e:
        raise Exception(f"Failed to load article data: {e}")

async def load_article_authors(article_id: int) -> list[RbaseAuthor]:
    """
    Load article authors from Rbase database
    """
    rbase_db_config = configuration.config.rbase_settings.get("database")
    try:
        pool = await get_mysql_pool(rbase_db_config)
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                sql = """
                SELECT au.id, au.cname, au.ename, 
                    aa.relation, aa.is_co_first, aa.is_co_last, au.mrgut_zhiku_id 
                FROM author au, article a, author_article aa
                WHERE a.id=aa.article_id AND au.id=aa.author_id AND aa.article_id = %s
                """
                await cursor.execute(sql, (article_id,))
                article_authors = await cursor.fetchall()
                results = []
                for author in article_authors:
                    ename = author["ename"] if author["ename"] else ""
                    cname = author["cname"] if author["cname"] else ""
                    name = cname if cname else ename
                    is_first_author = (author["relation"] == 1 or author["relation"] == 3) and author["is_co_first"] == 1
                    is_co_first_author = (author["relation"] == 1 or author["relation"] == 3) and author["is_co_first"] == 0
                    is_corresponding = (author["relation"] == 2 or author["relation"] == 3) and author["is_co_last"] == 1
                    is_co_corresponding = (author["relation"] == 2 or author["relation"] == 3) and author["is_co_first"] == 0
                    results.append(RbaseAuthor(
                        name=name, ename=ename, cname=cname, 
                        is_corresponding=is_corresponding, is_co_corresponding=is_co_corresponding, 
                        is_first_author=is_first_author, is_co_first_author=is_co_first_author))
                return results
    except Exception as e:
        raise Exception(f"Failed to process database data: {e}")
...
def log_raw_article_deleted(rbase_config: dict, raw_article_id: int, collection_name: str) -> int:
    ...
    try:
        with conn.cursor() as cursor:
            sql = "SELECT id, id_from, id_to FROM vector_db_data_log WHERE raw_article_id=%s AND collection=%s AND (operation=%s or operation=%s) AND status=1 ORDER BY id DESC"
            cursor.execute(sql, (raw_article_id, collection_name, VectorDbOperation.INSERT.value, VectorDbOperation.ABSTRACT.value))
            result = cursor.fetchone()
            if result:
                id_from = result["id_from"]
                id_to = result["id_to"]
                save_vector_db_log(rbase_config, raw_article_id, collection_name, operation=VectorDbOperationStr.DELETE, id_from=id_from, id_to=id_to)
                sql = "UPDATE vector_db_data_log SET status=0 WHERE id=%s"
...
def save_vector_db_log(rbase_config: dict, raw_article_id: int, collection_name: str, **kwargs) -> int:
    ...
    operation = kwargs.get('operation', VectorDbOperationStr.INSERT.value)
    operation_int = vector_db_operation_to_int(operation)
    ...
    try:
        with conn.cursor() as cursor:
            ...
            """
            # Use parameterized query to avoid string formatting issues
            cursor.execute(sql, (raw_article_id, collection_name, id_from, id_to, chunks, operation_int, 1))
            rt = cursor.lastrowid
            if operation == VectorDbOperationStr.INSERT or operation == VectorDbOperationStr.ABSTRACT:
                sql = "UPDATE vector_db_data_log SET status=0 WHERE raw_article_id=%s AND collection=%s AND (operation=%s OR operation=%s) AND id <> %s"
                cursor.execute(sql, (raw_article_id, collection_name, VectorDbOperation.INSERT.value, VectorDbOperation.ABSTRACT.value, rt))
            conn.commit()
    except Exception as e:
        # Close connection when exception occurs
        close_mysql_connection()
...
async def load_article_vector_db_log(raw_article_id: int, operation: VectorDbOperation, status: int = 1):
    try:
        pool = await get_mysql_pool(configuration.config.rbase_settings.get("database"))
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                sql = "SELECT id, collection, id_from, id_to, chunks, operation, status FROM vector_db_data_log WHERE raw_article_id=%s AND operation=%s AND status=%s"
                await cursor.execute(sql, (raw_article_id, operation.value, status))
                result = await cursor.fetchone()
                return result
    except Exception as e:
        raise Exception(f"Failed to load article vector db log: {e}")


async def get_authors_by_name(name: str) -> List[int]:
    """
    Get author IDs by author name
    
    Args:
        name: Author name (Chinese or English)
        
    Returns:
        List[int]: List of author IDs
    """
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cursor:
                ...
                if is_chinese:
                    # Chinese name: exact match on cname field first
                    sql = "SELECT id FROM author WHERE cname = %s AND status = 1"
                    await cursor.execute(sql, (name,))
                    results = await cursor.fetchall()
                    
                    if not results:
                        # If no exact match, use fuzzy matching
                        sql = "SELECT id FROM author WHERE cname LIKE %s AND status = 1"
                        await cursor.execute(sql, (f"%{name}%",))
                        results = await cursor.fetchall()
                else:
                    # English name: exact match on ename and ename2 fields first
                    sql = "SELECT id FROM author WHERE (ename = %s OR ename2 = %s) AND status = 1"
                    await cursor.execute(sql, (name, name))
                    results = await cursor.fetchall()
                    
                    if not results:
                        # If no exact match, use fuzzy matching
                        sql = "SELECT id FROM author WHERE (ename LIKE %s OR ename2 LIKE %s) AND status = 1"
                        await cursor.execute(sql, (f"%{name}%", f"%{name}%"))
                        results = await cursor.fetchall()
                
                # Extract all matching author IDs
                author_ids = [row["id"] for row in results]
                
                return author_ids
                
    except Exception as e:
        raise Exception(f"Failed to get authors by name: {e}")


async def get_term_tree_node_children_ids(rbase_config: dict, term_tree_node_id: int, tree_id: int = 0, **kwargs) -> list[int]:
    """
    Get term tree node children ids from term_tree_node table
    ...
    Args:
        ...
        offset: Query start position
        limit: Query limit count
        include_self: Whether to include the term tree node id itself

    Returns:
        List of term tree node children ids
    """
    ...
    recursive_level = kwargs.get('recursive_level', 1)
    # Get MySQL connection
    rbase_db_config = rbase_config.get('database', {})
    conn = get_mysql_connection(rbase_db_config)
    ...
    try:
        with conn.cursor() as cursor:
            ...
            WHERE `status`=1 AND `parent_node_id`=%s 
            """
            params = [term_tree_node_id]
            if tree_id > 0:
                sql += " AND `tree_id`=%s"
                params.append(tree_id)
            sql += " ORDER BY `id` ASC"
            cursor.execute(sql, tuple(params))
            term_tree_node_ids = cursor.fetchall()
            children_ids = [row["id"] for row in term_tree_node_ids]
            if recursive_level > 1:
                sub_children_results = []
                for child_id in children_ids:
                    sub_children_ids = await get_term_tree_node_children_ids(
                        rbase_config, child_id, tree_id=tree_id, 
                        offset=0, limit=0, include_self=False, 
                        recursive_level=recursive_level - 1)
                    sub_children_results.extend(sub_children_ids)
                children_ids.extend(sub_children_results)
            # sort and unique children_ids
            children_ids = sorted(set(children_ids))
            if include_self:
                children_ids.insert(0, term_tree_node_id)
            if limit > 0 and offset >= 0:
                offset = min(offset, len(children_ids))
                tail = min(offset+limit, len(children_ids))
                children_ids = children_ids[offset:tail]
            return children_ids
    except Exception as e:
        # Close connection when exception occurs
        close_mysql_connection()
